...
/*   Updated: 2020/10/22 19:05:32 by grigaux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		past_line_checker(t_gameinf *game, int y)
{
	...
	{
		if (game->map[y - 1][x] == '0' && !(game->map[y][x] ||
			ft_isspace(game->map[y][x])))
			return (0);
		x++;
	}
	return (1);
...
int		line_checker(t_gameinf *game, int y)
{
	...
	while (game->map[y][x])
	{
		if (ft_isspace(game->map[y][x]) && game->map[y][x - 1] &&
			game->map[y][x - 1] == '0')
			return (0);
		if (!ft_isspace(game->map[y][x]))
			last_c = game->map[y][x];
		if (game->map[y][x] == '0' && (x == 0 || !(game->map[y - 1][x] ||
			ft_isspace(game->map[y][x - 1]) ||
